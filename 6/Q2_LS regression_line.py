# -*- coding: utf-8 -*-
"""
Created on Sat Sep 10 10:40:46 2022

@author: Y P Zhu

Least Squares (LS) algorithm / Maximum Likelihood (ML) algorithm
Realization of linear regression - Line 
"""
import os
...
"""
RegsMat = np.zeros((data_length, 2)) # Polynomial order is 1
for i in range(data_length):
    RegsMat[i,:] = [1, x_sample[i]]

# print( np.linalg.inv(RegsMat.T @ RegsMat) @ RegsMat.T )
...
for i in range(1000):
    RegsVec = np.array([1, xp_sample[i]])
...
# np.random.seed()
# Weights_init = np.random.randn(2) # Polynomial order is 1 [x 1]
