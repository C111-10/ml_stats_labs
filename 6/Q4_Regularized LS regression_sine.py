...
Regularized Least Squares (RLS) algorithm / Bayesian Linear Regression (BLR) algorithm
Solutions to overfitting problems - Sine
"""
import os
...
loaded_data = np.loadtxt('Q4_data.csv', delimiter=',', skiprows=1)  # Skip the header row
...
"""
---- RLS/BLR regresion based on Python coding
"""
RegsMat = np.zeros((data_length, 11)) # Polynomial order is 10
for i in range(data_length):
    RegsMat[i,:] = ( [1, x_sample[i], x_sample[i]**2, x_sample[i]**3,
                    x_sample[i]**4, x_sample[i]**5, x_sample[i]**6,
                    x_sample[i]**7, x_sample[i]**8, x_sample[i]**9,
                    x_sample[i]**10] )
    
I = np.eye(11)
lamda=1e-3
Weights = np.linalg.inv(RegsMat.T @ RegsMat + lamda * I) @ RegsMat.T @ y_noi # The regularized LS algorithm

xp_sample = np.linspace(0, 1, 1000) # Select 1000 points as the x_p
y_pred = np.zeros((1000,))

for i in range(1000):
    RegsVec = ( np.array([1, xp_sample[i], xp_sample[i]**2, xp_sample[i]**3,
                    xp_sample[i]**4, xp_sample[i]**5, xp_sample[i]**6,
                    xp_sample[i]**7, xp_sample[i]**8, xp_sample[i]**9,
                    xp_sample[i]**10]) )
...
"""
---- RLS regresion based on Python package
"""
...
# np.random.seed()
# Weights_init = np.random.randn(11) # Polynomial order is 10 [x^10 ... x 1]
